...
namespace cfclapi.Controllers.ledgers.electrical
{
    ...
    public class PUR203Controller : ControllerBase
    {
        private string _connectionString;

        private readonly itsppisapi.Models.DataContext _context;
        public PUR203Controller(itsppisapi.Models.DataContext context)
        {
            _context = context;
        }

        [HttpGet("{mnth}/{dept}")]
        public async Task<DataSet> get(string mnth, string dept)
        {
            try
            {
                string strqry = "[PPIS].[PPU_P_UR2_DR_PLANT_MAINTENANCE_PUR203]";
                ...
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(strqry, sql))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@IN_DEPT", dept));
                        await sql.OpenAsync();
