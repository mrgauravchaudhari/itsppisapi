...
namespace cfclapi.Controllers.ledgers.electrical
{
    ...
    public class POR092Controller : ControllerBase
    {
        private string _connectionString;

        private readonly itsppisapi.Models.DataContext _context;
        public POR092Controller(itsppisapi.Models.DataContext context)
        {
            _context = context;
        }
        ...
        public async Task<DataSet> get(string month)
        {
            try
            {
               string strqry = "[PPIS].PPU_P_OU1_YL_BUDGET_ACTUAL_QTY_CONS_POR092";
                ...
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand(strqry, sql))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@IN_YR", month));
                        await sql.OpenAsync();
