...
{
    public class PAS211Repository
    {
        private readonly string _connectionString;
        public PAS211Repository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("DBConnection");
        }

        private PAS211Model MapToValue(SqlDataReader reader)
        {
            return new PAS211Model()
            {
        ...
        public async Task<PAS211Model> putData(TransactionDateBtnDto value)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_AM2_GET_PPT_OU_CT_DETAILS", sql))
                {
        ...
        public async Task saveData(PAS211SaveDto value)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_AM2_SAVE_PPT_OU_CT_DETAILS", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
