...
{
    public class PBS011Repository
    {
        private readonly string _connectionString;
        public PBS011Repository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("DBConnection");
        }

        private PBS011Model MapToValue(SqlDataReader reader)
        {
            return new PBS011Model()
            {
        ...
        public async Task<PBS011Model> putData(string IN_DATE, char IN_BTN)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_BG2_GET_PPT_BG_NEEM_OIL_DETAILS_G3", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@IN_DATE", IN_DATE));
                    cmd.Parameters.Add(new SqlParameter("@IN_BTN", IN_BTN));
                    PBS011Model response = null;
                    await sql.OpenAsync();
        ...
        public async Task saveData(PBS011Dto value)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_BG2_SAVE_PPT_BG_NEEM_OIL_DETAILS_G3", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
