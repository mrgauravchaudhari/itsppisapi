...
{
    public class PMS201Repository
    {
        private readonly string _connectionString;
        public PMS201Repository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("DBConnection");
        }
        private PMS201Model MapToValue(SqlDataReader reader)
        {
            return new PMS201Model()
            {
        ...
        public async Task<PMS201Model> putData(string IN_DATE, char IN_BTN)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_MG_GET_PPT_MG_VPO_REMARKS", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@IN_DATE", IN_DATE));
                    cmd.Parameters.Add(new SqlParameter("@IN_BTN", IN_BTN));
                    PMS201Model response = null;
                    await sql.OpenAsync();
        ...
        public async Task saveData(PMS201SaveDto value)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_MG_SAVE_PPT_MG_VPO_REMARKS", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
