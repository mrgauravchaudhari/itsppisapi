...
{
    public class PGS006Repository
    {
        private readonly string _connectionString;
        public PGS006Repository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("DBConnection");
        }

        private PGS006Model MapToValueMM(SqlDataReader reader)
        {
            return new PGS006Model()
            {
        ...
        public async Task<List<PGS006Model>> putData()
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_GET_PPM_GL_CONST_PARAMS", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    var response = new List<PGS006Model>();
                    await sql.OpenAsync();
        ...
        public async Task saveData(PGS006SaveDto value)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_SAVE_PPM_GL_CONST_PARAMS", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
