...
{
    public class PLR006ReportRepository
    {
        private readonly string _connectionString;
        public PLR006ReportRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("DBConnection");
        }

        private PLR006ReportModel MapToValue(SqlDataReader reader)
        {
            return new PLR006ReportModel()
            {
        ...
        public async Task<List<PLR006ReportModel>> GetById(string TDATE)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_LB_DAILY_LAB_ANALS_GP2_D_RPT", sql))
                {
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@IN_DATE", TDATE));
                    var response = new List<PLR006ReportModel>();
                    await sql.OpenAsync();
