...
{
    public class PLR005ReportRepository
    {
        private readonly string _connectionString;
        public PLR005ReportRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("DBConnection");
        }

        private PLR005ReportModel MapToValue(SqlDataReader reader)
        {
            return new PLR005ReportModel()
            {
        ...
        public async Task<List<PLR005ReportModel>> GetById(string TDATE)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("PPIS.PPU_P_LB_SAMPLE_ATTR_ANALYS_WISE_D_RPT", sql))
                {
                    ...
                    var response = new List<PLR005ReportModel>();
                    await sql.OpenAsync();
